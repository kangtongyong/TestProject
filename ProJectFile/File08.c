...
void file08(void)
{
	float cm, kg = 0.0;						//	신장(cm)과 체중(kg) 저장 변수
	float normal_kg_result = 0.0;			//	표준체중 계산 결과 저장 변수
...
{
	float normalkg_result;								//	표준 체중 저장 변수
...
{
	float bodymass_result;								//	신체질량지수 저장 변수

	float floatcm = CM / 100;							//	신장을 m 단위로 변환
...
{
	float obesity_result = 0.0;						//	비만도 결과 저장 변수
